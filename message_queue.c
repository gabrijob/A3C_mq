...
void do_queue(int msgperbatch,int max_effective_data_to_send_per_window, int duration, int sending_port, int receiving_port, int io_threads,int nb_sending_sockets, int msgsize, int threshold, int qosmin, char** split_ipqueue, int controll, int qosmax, int loss, int window)
{
    ...
    /*if(controll == 0){	
    	saqn_time_csv = fopen("/tmp/SAQN_TIME.csv", "w");
    	fprintf(saqn_time_csv, "TIMESTAMP \t,\t SAQN_PROC_TIME\n");
    	fflush(saqn_time_csv);
    	fclose(saqn_time_csv);
    }*/
...
//	pps1++;
   ...
   if (controll == 0) {
		...
		printf("\nStarting Parameter Server");
    		p_server = parameter_server_proc(180);
	#endif
    ...
    worker = create_worker(start_state,0);
#endif
...
#endif
#ifdef A3C_RUN
	if (PyImport_AppendInittab("A3CProcessess", PyInit_A3Cprocesses) == -1) {   
		fprintf(stderr, "Error: could not extend in-built modules table\n");
		exit(1);
	}
#endif
...
#endif
#ifdef A3C_RUN
	pmodule = PyImport_ImportModule("A3CProcessess");
	if (!pmodule) {
	       	PyErr_Print();
		fprintf(stderr, "Error: could not import module 'A3CProcessess'\n");
        	PyMem_RawFree(program);
		Py_Finalize();
		exit(1);
	}
